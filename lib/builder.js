...
SpriteSheetBuilder = (function() {
  ...
  SpriteSheetBuilder.pngcrush = function(image, callback) {
    return SpriteSheetBuilder.supportsPngcrush(function(supported) {
      ...
      if (supported) {
        ...
        return exec("pngcrush -reduce " + image + " " + crushed + " && " + utils.environment.mv + " " + crushed + " " + image, function(error, stdout, stderr) {
          return callback();
        });
      } else {
        return callback();
  ...
  function SpriteSheetBuilder(options) {
    ...
    if (options.outputCss) {
	  this.outputStyleDirectoryPath = (options.outputStyleDirectoryPath || this.outputDirectory);
      this.outputStyleFilePath = [this.outputStyleDirectoryPath, options.outputCss].join(separator);
    }
  }
  ...
  SpriteSheetBuilder.prototype.build = function(done) {
    ...
    if (Object.keys(this.outputConfigurations).length === 0) {
      this.addConfiguration("default", {
        pixelRatio: 1,
		restrictAxis: this.options.restrictAxis
      });
    }
    this.configs = [];
    baseConfig = null;
...
SpriteSheetConfiguration = (function() {
  ...
  function SpriteSheetConfiguration(files, options) {
	...
	this.options.restrictAxis = options.restrictAxis;
